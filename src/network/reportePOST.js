...
const reportePOST =  async (id_cliente, userToken, navigate, range) =>{
    const now = new Date();
    ...
    const filename = `Reporte_${year}${month}${day}_${hours}${minutes}${seconds}.pdf`;
    ...
    if(range.startDate != undefined){
      ...
      console.log('Se selecciono un rango de fechas reportePOST')
    }

    const result = await FileSystem.downloadAsync(
      `${BASE_URL}/generarPDF?id_cliente=${id_cliente}&startDate=${startDate}&endDate=${endDate}&fechaSeleccionada=${fechaSeleccionada.toString()}`,
...
const showAlert = (uri) =>{
  Alert.alert('Éxito', 'Reporte generado con exito', [
    {
      text: 'Compartir',
      onPress: () => {
      ...
      await FileSystem.StorageAccessFramework.createFileAsync(directoryUri, filename, mimetype)
        .then(async (NewSAFuri) => {
          ...
          navigate('NavigationTab',{
            screen: 'Cobranzas',
            params: { mensaje: 'Reporte Descargado con exito' },
          })
          showAlert(uri)
        })
